...
def process_bert(data, tokenizer, vocab):
    ...
    for ins_id, instance in tqdm.tqdm(enumerate(data), total=len(data)):
        ...
        _inputs = [tokenizer.cls_token_id] + tokenizer.convert_tokens_to_ids(tokens_list) + [tokenizer.sep_token_id]
        ...
        for event in events:
            ...
            t_s, t_e = trigger.get("start", -1), trigger.get("end", -1)
            # event_type = vocab.label2id(event["type"], "tri")
            event_type = vocab.label2id(event.get("type", event.get("event_type", "UNK")), "tri")

            t_s = max(0, min(t_s, _tri_labels.shape[1] - 1))
            t_e = max(0, min(t_e, _tri_labels.shape[2] - 1))
            if t_s > t_e:
                continue
            ...
            for arg in args:
                if "span" in arg:
                    ...
                    a_s, a_e = arg.get("start", -1), arg.get("end", -1)
                # ⚙️ Thêm kiểm tra biên
                a_s = max(0, min(a_s, _arg_labels.shape[1] - 1))
                a_e = max(0, min(a_e, _arg_labels.shape[2] - 1))
                if a_s > a_e:
                    continue
