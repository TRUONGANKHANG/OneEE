...
class Config:
    def __init__(self, args):
        ...
        ty_args_path = f"./data/{self.dataset}/ty_args.json"
        if os.path.exists(ty_args_path):
            with open(ty_args_path, "r", encoding="utf-8") as f:
                self.tri_args = json.load(f)
        else:
            print(f"[WARN] File ty_args.json not found at {ty_args_path}. Using empty mapping.")
            self.tri_args = {}
